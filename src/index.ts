...
function assertFail(f:boolean, reason:string) {
  if(!f) {
...
function beginSampling(adxlWire:i2c) {
...
function startup():Promise<i2c> {

  return new Promise((resolve) => {
    var address = 0x18;
    var scanWire = new i2c(address, {device: '/dev/i2c-1'}); // point to your i2c address, debug provides REPL interface
    
    scanWire.scan(function(err, data:number[]) {
      ...
      const adxlWire = new i2c(addressofAdxl, {device: '/dev/i2c-1'});
...
process.on('SIGINT', function () {
  console.log("exiting!");
  const channels:{[key:string]:{tm:number, sample:number}[]} = {
    "x-raw": [],
    "y-raw": [],
    "z-raw": [],
    "total-raw": [],
  ...
  fs.writeFileSync('./out.signals.out', lines.join('\n'));
  process.exit(0);
});